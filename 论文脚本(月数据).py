...
model = sm.tsa.ARIMA(xdata,(5,0,1)).fit()
print(model.summary())
model.fittedvalues
pd.DataFrame({'实际值':xdata,'arima':model.fittedvalues}).to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\个人所有填写表格\谢杰同等学力\论文\论文初稿\输出结果\月输出结果arima.xlsx')
#每个质保评分
#xdata=xdata[1:]
...
pd.concat([sup_water_train,forecast['yhat'][:-7]],axis=1).to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\个人所有填写表格\谢杰同等学力\论文\论文初稿\输出结果\月输出结果prophet.xlsx')
...
result['预测'] = inv_yhat
result.to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\论文初稿\输出结果\月输出结果lstm(提高了准确度).xlsx')
...
data_m = pd.read_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\月实验数据.xlsx')
data_m.info()
data_m[['供水总量','最高温度','平均温度','最低温度','最大相对湿度','平均相对湿度','最小相对湿度']].corrwith(data_m['供水总量']).sort_values(ascending=False)
data_m[['供水总量','最高温度','平均温度','最低温度','最大相对湿度','平均相对湿度','最小相对湿度']].corr()

pd.plotting.scatter_matrix(data_m[['供水总量','最高温度','平均温度','最低温度','最大相对湿度','平均相对湿度','最小相对湿度']],figsize=(12,8),alpha=0.2)


xdata = data_m['供水总量']

np.std(xdata) / np.mean(xdata) * 100
...
for parameters in parameters_list:
    try:
        ...
        model = sm.tsa.SARIMAX(xdata,order=(5, 0, 2),
                                 seasonal_order=(parameters[0], 0, parameters[1], 9),
    ...
    if R2 > best_R2:
        ...
        results.append([parameters, R2])
 
result_table = pd.DataFrame(results)
result_table.columns = ['parameters', 'R2']
print(result_table.sort_values(by='R2', ascending=False).head())
...
model =sm.tsa.ARIMA(xdata,order=(5,0,1),exog=data_m.iloc[:,[6]]).fit()  

# y_hat_avg['SARIMA'] = fit.predict(start="2020-2-14", end="2020-2-16", dynamic=True)
#model = sm.tsa.ARIMA(xdata,(1,0,3)).fit()
print(model.summary())
model.fittedvalues
pd.DataFrame({'实际值':xdata,'arima':model.fittedvalues}).to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\论文初稿\输出结果\外变量平均湿度arima.xlsx')
#每个质保评分
abs_=(xdata- model.fittedvalues).abs()
mae=abs_.mean()#Mean Absolute Error ，平均绝对误差
rmse= np.sqrt((abs_**2).mean()) #Root Mean Square Error,均方根误差
...
#m.add_seasonality(name='weekly', period=168, fourier_order=10, prior_scale=10) #fourier_order越大，对周期变化的拟合越细致也越容易过拟合。
#m.add_seasonality(name='hourly', period=24, fourier_order=10, prior_scale=10)#prior_scale越大，对于目标变量影响越大。
m.add_regressor('平均相对湿度',prior_scale=10,mode='multiplicative') #回归量采用乘法模型
#时间戳的格式应该是YYYY-MM-DD - HH:MM:SS。当使用子日(日以下)数据时，日季节性将自动匹配
...
#future = m.make_future_dataframe(periods=3,freq = 'MS', 
                                 #include_history = False
#                                 )
forecast = m.predict(sup_water_train)
m.plot_components(forecast)
m.plot(forecast)
...
yhat_tmp = trainyhat.reshape((-1,n_pre_features))#2为预测指标数
#对其反转矩阵,
inv_yhat = concatenate((yhat_tmp, scaled[9:9+yhat_tmp.shape[0],1:]), axis=1)#截取数据集相同的行数对齐
##视模型对于scaled截取预测数据的起点，的长度要和预测数据一致
...
result['预测'] = inv_yhat
result.to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\论文初稿\输出结果\外变量平均湿度LSTM.xlsx')
...
data_m = pd.read_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\月实验数据2.xlsx')
...
forecast.to_excel(r'd:\BaiduSyncdisk\谢杰\My Documents\谢杰同等学力\论文\论文初稿\输出结果\prophet2023年1-8月供水量检验.xlsx')
