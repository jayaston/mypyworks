...
#shuju_df['QUOTA_NAME'].replace(['取水量','入厂取水量','供水总量'],['0取水量','1入厂取水量','2供水总量'],inplace=True)
...
test.loc['供水总量_江村水厂'] = test.loc['供水总量_江村一厂'] + test.loc['供水总量_江村二厂'] 
...
test.loc['供水总量_广州自来水公司'] = test.loc['供水总量_北部水厂'] + test.loc['供水总量_南洲水厂'] + test.loc['供水总量_新塘水厂'] + test.loc['供水总量_石门水厂'] \
      + test.loc['供水总量_西村水厂'] + test.loc['供水总量_西洲水厂'] + test.loc['供水总量_江村水厂']

test.drop(['供水总量_江村一厂','供水总量_江村二厂','入厂取水量_江村一厂','入厂取水量_江村二厂'],inplace=True)
...
sorted_data = sorted_data.assign(开发区东4 = sorted_data.get('天河开发区路口DN600正向累计',0),
                 ...
                 黄岐 = sorted_data.get('南9（黄岐）',0),
                  公司取供比 = sorted_data.get('取水量_广州自来水公司',0) / sorted_data.get('供水总量_广州自来水公司',0),
                   西村取供比 = sorted_data.get('入厂取水量_西村水厂',0) / sorted_data.get('供水总量_西村水厂',0),
                   石门取供比 = sorted_data.get('入厂取水量_石门水厂',0) / sorted_data.get('供水总量_石门水厂',0),
                   江村取供比 = sorted_data.get('入厂取水量_江村水厂',0) / sorted_data.get('供水总量_江村水厂',0),
                   北部取供比 = sorted_data.get('入厂取水量_北部水厂',0) / sorted_data.get('供水总量_北部水厂',0),
                   新塘取供比 = sorted_data.get('入厂取水量_新塘水厂',0) / sorted_data.get('供水总量_新塘水厂',0),
                   西洲取供比 = sorted_data.get('入厂取水量_西洲水厂',0) / sorted_data.get('供水总量_西洲水厂',0),
                   南洲取供比 = sorted_data.get('入厂取水量_南洲水厂',0) / sorted_data.get('供水总量_南洲水厂',0),
                 )
...
sorted_data = sorted_data.reindex(columns=['取水量_广州自来水公司', '入厂取水量_西村水厂','入厂取水量_石门水厂','入厂取水量_江村水厂','入厂取水量_北部水厂','入厂取水量_新塘水厂', '入厂取水量_西洲水厂', '入厂取水量_南洲水厂',
                                           '最高温','最低温','平均温',  
    '供水总量_广州自来水公司','供水总量_西村水厂','供水总量_石门水厂','供水总量_江村水厂', '供水总量_北部水厂','供水总量_新塘水厂', '供水总量_西洲水厂', '供水总量_南洲水厂', '供水总量_穗云水厂',
...
sorted_data.to_excel(r'C:\Users\XieJie\mypyworks\输出\2022年10月分区供水.xlsx')
